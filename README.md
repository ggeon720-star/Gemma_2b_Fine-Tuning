...
# Datasets
 1. 심화 법률 데이터 선정 이유
 데이터셋을 선정하기 위해 우선 학습시킬 base-model을 선정하였습니다. 저희가 만들고자 하는 모델은 한국어 데이터셋을 이용해 한국 법률 정보를 습득하고 이를 한국어로 출력할 수 있는 모델이기에,
base-model로 구글의 gemma-2b-it모델을 그대로 사용하기보다, 한국어 데이터로 한 번 파인튜닝된 모델을 사용하는 것이 좋을 것이라고 판단했습니다.
따라서, 한국어 법률 기초 정보로 학습된 모델을 찾아 base-model로 선정한 후, 심화 법률 데이터를 학습시키면 효율적인 법률 LLM모델을 만들 수 있다고 판단하였습니다.
hugging face에 공개된 AI 허브의 법률안 검토 보고서 요약 데이터셋을 기반으로 학습된 모델 RangDev/gemma-2b-it-legal-sum-ko을 base-model을 base-model로 최종 선정하게 되었고 이에 따라 학습할 데이터셋을 심화 법률 데이터셋으로 선택하게 되었습니다.

 2. 법률/규정 텍스트 분석 데이터(판례)
모델 학습에 사용할 데이터로 AI허브의 법률/규정 텍스트 분석 데이터를 사용하기로 결정하였습니다. 법률/규정 텍스트 분석 데이터는 활용도가 높은 판례 데이터를 수집하고 판례의 주요 내용을 추출요약해 질의응답 셋을 작성한 데이터로, 판결에 큰 영향을 주는 판례 들을 추출 요약한 데이터와 질의-응답 데이터로 구분하고 라벨링한 데이터 입니다. 데이터가 추출 요약 66,511건, 질의-응답	90,944건으로 상당히 많은 편이기에 효율적인 데이터셋의 일부만 선정하여 학습하기로 결정하였습니다.
우선, 질의-응답 90944건 중 상황별 판례 데이터를 질의-응답으로 바꾼 데이터는 총 70,784건으로 많은 부분을 차지하지만, json데이터 파일을 살펴보았을 때, 응답이 부정, 긍정, 불명으로 3가지 대답으로 한정되어 있어 법률 정보에 대한 정확한 응답을 주기에는 부적절한 데이터셋이라고 판단하여 질의-응답 데이터 중 나머지 데이터인 QA데이터20,160건을 선정하여 학습하기로 결정하였습니다.
QA데이터셋은 질문인 instruction과 대답인 answer로 나뉘어져 LLM동작과 직접적인 연관이 있는 학습 방법이므로 학습 효율이 좋다는 점에서 선정하게 되었습니다.
